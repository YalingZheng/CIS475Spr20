...
## The A5 / 1 algorithm is used in 2 G GSM for over - the - air encryption of voice and SMS
...
keybytes = [BitVector(hexstring = x).reverse() for x in ['12', '23', '45', '67', '89', 'ab', 'cd', 'ef']] #(9)
...
def setupkey():    #(33)
 ...
 # of each register depend on the majority voting of the clocking bits:
  for i in range(100): #(40)
      maj = majority()   #(41)
      if (R1[8] != 0) == maj:
         clockone(R1, R1TAPS)   #(42)
      if (R2[10] != 0) == maj:
         clockone(R2, R2TAPS)  #(43)
      if (R3[10] != 0) == maj:
...
def run():  #(45)
  ...
  for i in range(228): #(48)
    maj = majority()   #(49)
    if (R1[8] != 0) == maj:
       clockone(R1, R1TAPS)   #(50)
    if (R2[10] != 0) == maj:
       clockone(R2, R2TAPS)  #(51)
    if (R3[10] != 0) == maj:
       clockone(R3, R3TAPS)  #(52)
...
goodAtoB = [BitVector(hexstring = x) for x in ['53', '4e', 'aa', '58', '2f', 'e8', '15', '1a', 'b6', 'e1', '85', '5a', '72', '8c', '00']]  #(60)
goodBtoA = [BitVector(hexstring = x) for x in ['24', 'fd', '35', 'a3', '5d', '5f', 'b6', '52', '6d', '32', 'f9', '06', 'df', '1a', 'c0']]   #(61)
