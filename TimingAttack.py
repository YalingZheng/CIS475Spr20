...
##
## CAVEATS:
## This simple implementation is based on one possible
...
class TimingAttack( object ): #(I1)
    ...
    def correlate(self, series1, series2): #(C1)
        ...
        mseries1_squared, mseries2_squared = [x**2 for x in mseries1], [x**2 for x in mseries2] #(C7)
        correlation = sum(products) / math.sqrt(sum(mseries1_squared) * sum(mseries2_squared))  #(C8)
    ...
    def find_next_bit_of_private_key(self, list_of_previous_bits): #(F1)
        ...
        if B in self.correlations_cache: #(F7)
            correlation0 = self.correlations_cache[B]
        else: #(F9)
            ...
            correlation0 = self.correlate(self.times_taken_for_messages, times_for_partial_exponentiation) #(F22)
        ...
        if B in self.correlations_cache: #(F29)
            ...
            correlation1 = self.correlate(self.times_taken_for_messages, times_for_partial_exponentiation) #(F43)
...
if __name__ == '__main__':
    ...
    for i in range(10): #(A8)
        ...
        if discovered_bits: #(A23)
            modulus_to_discovered_bits[i] = (modulus, ''.join(map(str, list(reversed(discovered_bits))))) #(A24)
        print "\n SUCCESS!!!!!!!" #(A25)
